/* Official training guides — detailed for brand-new staff (v3.0.0)
...
window.renderGuideDetailHtml = function (key) {
  ...
  html += '</ul></div>';
  html += '<p style="font-weight:600;margin-bottom:6px">' + t(labels.done) + '</p><div class="checklist" id="guide-done-list">';
  list(g.done).forEach(function (x, i) {
    html += '<label style="cursor:pointer" onclick="if(typeof openGuideDoneDetail===\'function\')openGuideDoneDetail(\'' + key + '\',' + i + ')"><input type="checkbox" onclick="event.stopPropagation()"> ' + x + '</label>';
  });
  html += '</div>';
  ...
  html += '<div id="guide-goal-text" class="hidden" style="display:none">' + t(g.goal) + '</div>';
  return html;
};

window.openGuideDoneDetail = function (guideKey, index) {
  const g = window.CF_GUIDE_DETAIL && window.CF_GUIDE_DETAIL[guideKey];
  if (!g) return;
  const L = typeof currentLang !== 'undefined' ? currentLang : 'en';
  const details = (g.doneDetails && (g.doneDetails[L] || g.doneDetails.en)) || [];
  const doneArr = (g.done && (g.done[L] || g.done.en)) || [];
  const item = details[index] || { title: doneArr[index] || '', body: doneArr[index] || '' };
  const readLabel = { ko: '읽어주기', en: 'Read aloud', ja: '読み上げ', es: 'Leer' }[L] || 'Read aloud';
  const closeLabel = { ko: '닫기', en: 'Close', ja: '閉じる', es: 'Cerrar' }[L] || 'Close';
  const modal = document.getElementById('modal-content');
  if (!modal) return;
  modal.innerHTML =
    '<button class="close-modal" onclick="closeModal()">×</button>' +
    '<h3 style="margin-bottom:10px;padding-right:28px">' + (item.title || '') + '</h3>' +
    '<div class="script-box" id="done-detail-body" style="margin-bottom:14px;white-space:pre-wrap;line-height:1.55">' + (item.body || '') + '</div>' +
    '<div style="display:flex;gap:8px;flex-wrap:wrap">' +
    '<button class="btn" style="flex:1;min-width:120px" onclick="if(typeof speakText===\'function\')speakText(document.getElementById(\'done-detail-body\').innerText,this)">🔊 ' + readLabel + '</button>' +
    '<button class="btn btn-outline" style="flex:1;min-width:120px" onclick="closeModal()">' + closeLabel + '</button>' +
    '</div>';
