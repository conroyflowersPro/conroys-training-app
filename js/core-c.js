/* Conroy's Training App - content modals 4.0.0 */
    ...
    const taskDetails = {
      ...
      attachments: {
        ...
        checklist: {
          ko: ['White Sheet Product Detail 확인','필요한 첨부물 종류 파악 (풍선/초콜릿/인형/CardIsle)','첨부물 준비','상품에 올바르게 부착','모두 달려 있는지 최종 확인','확인 후 White Sheet 제거','Set As Awaiting Delivery'],
          en: ['Check White Sheet Product Detail','Identify required attachments (balloon/chocolate/plush/CardIsle)','Prepare the attachments','Attach them correctly to the product','Final check that everything is attached','Remove White Sheet only after confirmation','Set As Awaiting Delivery'],
          ja: ['White Sheet Product Detailを確認','必要な添付物を把握（バルーン/チョコ/ぬいぐるみ/CardIsle）','添付物を準備','商品に正しく取り付ける','すべて付いているか最終確認','確認後にWhite Sheetを外す','Set As Awaiting Delivery'],
          es: ['Revisar Product Detail de White Sheet','Identificar adjuntos necesarios (globo/chocolate/peluche/CardIsle)','Preparar los adjuntos','Colocarlos correctamente en el producto','Verificación final de que todo esté colocado','Quitar White Sheet solo después de confirmar','Set As Awaiting Delivery']
        }
      },
      end_day: {
        image: 'images/guides/cash.webp',
    ...
    function showContent(type) {
      ...
      if (type === 'attachments') {
        const titles = { ko: '첨부물 확인 · 부착 절차', en: 'Attachment Verification Process', ja: '添付物確認・取り付け手順', es: 'Proceso de verificación de adjuntos' };
        const intro = {
        ...
        const steps = {
          ko: ['White Sheet의 <strong>Product Detail</strong>을 확인한다.','필요한 첨부물 종류를 파악한다 (풍선, 초콜릿, 인형, CardIsle 카드 등).','해당 첨부물을 준비한다.','상품에 첨부물을 올바르게 부착한다.','모든 첨부물이 제대로 달려 있는지 최종 확인한다.','확인이 끝난 후에만 <strong>White Sheet를 제거</strong>한다.','BMS에서 <strong>Set As Awaiting Delivery</strong>로 상태를 변경한다.'],
          en: ['Check <strong>Product Detail</strong> on the White Sheet.','Identify required attachments (balloons, chocolates, plush, CardIsle card, etc.).','Prepare the needed attachments.','Attach them correctly to the product.','Do a final check that everything is properly attached.','Only after confirmation, <strong>remove the White Sheet</strong>.','In BMS, change status to <strong>Set As Awaiting Delivery</strong>.'],
          ja: ['White Sheetの<strong>Product Detail</strong>を確認する。','必要な添付物を把握する（バルーン、チョコ、ぬいぐるみ、CardIsleカードなど）。','該当する添付物を準備する。','商品に正しく取り付ける。','すべてきちんと付いているか最終確認する。','確認が終わってから<strong>White Sheetを外す</strong>。','BMSで<strong>Set As Awaiting Delivery</strong>に変更する。'],
          es: ['Revise el <strong>Product Detail</strong> de la White Sheet.','Identifique los adjuntos necesarios (globos, chocolates, peluche, tarjeta CardIsle, etc.).','Prepare los adjuntos correspondientes.','Colóquelos correctamente en el producto.','Haga una verificación final de que todo esté bien colocado.','Solo después de confirmar, <strong>quite la White Sheet</strong>.','En BMS, cambie el estado a <strong>Set As Awaiting Delivery</strong>.']
        };
        const cardisleNote = {
        ...
        const steps = {
          ko: ['새 주문 수신 (Messages / In Wire)','내용 검토 → Mark Read','In Wire에서 Accept','SuperTicket 출력','디자인 + 첨부물 작업','Set As Awaiting Delivery','배달 트립 생성'],
          en: ['New Order Received (Messages / In Wire)','Review → Mark Read','Accept Order (In Wire)','Print SuperTicket','Design + Attachments','Set As Awaiting Delivery','Create Delivery Trip'],
          ja: ['新規注文受信 (Messages / In Wire)','内容確認 → Mark Read','In WireでAccept','SuperTicket印刷','デザイン + 添付物','Set As Awaiting Delivery','配達トリップ作成'],
          es: ['Nuevo pedido recibido (Messages / In Wire)','Revisar → Mark Read','Accept en In Wire','Imprimir SuperTicket','Diseño + Adjuntos','Set As Awaiting Delivery','Crear viaje de entrega']
        };
        const list = steps[L] || steps.en;
        html += `<h3>${titles[L]||titles.en}</h3>
        ...
        const rules = {
          ko: ['Due Time 기준으로 주문 우선순위를 정한다.','인쇄된 Design Ticket 없이 디자인을 시작하지 않는다.','필요한 작업이 모두 끝날 때까지 Complete 하지 않는다.','배송일 변경 전에 반드시 Delivery Attempted 메시지를 보낸다.','확신이 없으면 먼저 매니저에게 문의한다.'],
          en: ['Always prioritize orders based on Due Time.','Never begin designing without a printed design ticket.','Do not complete an order until all required work is finished.','Always send a Delivery Attempted message before changing the delivery date.','If unsure, contact a manager before taking action.'],
          ja: ['Due Timeを基準に注文の優先順位を決める。','印刷されたDesign Ticketなしにデザインを始めない。','必要な作業がすべて終わるまでCompleteしない。','配達日変更前に必ずDelivery Attemptedメッセージを送る。','確信がなければ先にマネージャーに相談する。'],
          es: ['Priorice siempre los pedidos según Due Time.','Nunca comience a diseñar sin un ticket de diseño impreso.','No complete un pedido hasta terminar todo el trabajo requerido.','Siempre envíe un mensaje Delivery Attempted antes de cambiar la fecha de entrega.','Si no está seguro, consulte a un gerente antes de actuar.']
        };
