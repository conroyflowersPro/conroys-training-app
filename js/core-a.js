/* Conroy's Training App - core part A 4.0.2 */
// ========== AUTH / USERS ==========
    const DEFAULT_USERS = [
    ...
    function buildDailyRoutineSpeech() {
      ...
      return ({
        ko: '지금 「' + title + '」을(를) 하시면 됩니다.' + (desc ? ' ' + desc : ''),
        en: 'Right now, please do: ' + title + '.' + (desc ? ' ' + desc : ''),
        ja: '今は「' + title + '」を進めてください。' + (desc ? ' ' + desc : ''),
        es: 'Ahora haga: ' + title + '.' + (desc ? ' ' + desc : '')
      })[L];
    }
    ...
    function showWelcomeInDock() {
      ...
      let msg;
      // Daily-routine coach: always name + what to do now (not open-ended help desk)
      if (!alreadyGreeted) {
